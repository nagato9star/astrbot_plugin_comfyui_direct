...
| `comfyui_edit` | 用独立 `edit_workflow` 路由修改当前或引用消息中的图片；可选 `resolution`、`custom_size`、`width`、`height`，无附图时可用本插件上次生成的图片 |
